...
int main() {
    ...
    while (true) {
        ...
        if (shared_mem->valid == 1 && shared_mem->seq_number != last_seq_number) {
            last_seq_number = shared_mem->seq_number;
            printf("Received data from Python [Seq: %d]: %s\n", shared_mem->seq_number, shared_mem->message);
            shared_mem->valid = 0;  // 데이터 사용 후 무효 상태로 설정
        }
